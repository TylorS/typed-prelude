...
export const TestId = Key<Test>('TestId')
export type TestId = t.TypeOf<typeof TestId>
...
export const TestCase: t.Type<TestCase> = t.record(
  {
    ...
    runTest: t.refinement(
      t.Function,
      (f): f is TestCase['runTest'] => f.length === 0 && isGenerator(f()),
    ),
  },
  `TestCase`,
)
...
export type ModifierOf<A> = A extends Test ? ConfigOf<A>['modifier'] : 'Default'
