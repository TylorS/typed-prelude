...
import { createJsonRpcHandler } from './createJsonRpcHandler'
...
export const test = describe(`createJsonRpcHandler`, [
  describe(`requests`, [
    given(`request and notification handlers`, [
      ...
      it(`keeps open request stats`, async ({ equal }) => {
        ...
        const promise = handler.sendRequest(request)
      ...
      it(`sends method not found response when unknown request comes in`, async ({ equal }) => {
        ...
        const response = await handler.sendRequest(request as any)
...
function createClientConnectHandler({
  ...
  const handler = createJsonRpcHandler(
    {
      'client/connect': createRequestHandler<ClientConnectRequest, ClientConnectResponse>(
        ({
          clientId,
          capabilities,
